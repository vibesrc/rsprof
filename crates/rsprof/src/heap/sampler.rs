...
impl HeapSampler {
    ...
    pub fn new(pid: u32, exe_path: &Path) -> Result<Self> {
        ...
        let open_skel = skel_builder.open(&mut open_object).map_err(|e| {
            Error::Bpf(format!("Failed to open BPF skeleton: {}. Make sure clang and libbpf-dev are installed.", e))
        })?;

        // Load the BPF program first
        let skel = open_skel.load().map_err(|e| {
            Error::Bpf(format!("Failed to load BPF program: {}", e))
        })?;

        // PID filtering disabled - uprobe attachment to specific binary is sufficient
        let key: u32 = 0;
        let pid_zero: u32 = 0;
        skel.maps.target_pid_map.update(&key.to_ne_bytes(), &pid_zero.to_ne_bytes(), MapFlags::ANY)
        ...
        if let Some(alloc_offset) = symbols.rust_alloc {
            // Entry probe
            let link = skel.progs.uprobe_rust_alloc.attach_uprobe(
                false, // not retprobe
                pid as i32,
                exe_path,
            ...
            // Return probe
            let link = skel.progs.uretprobe_rust_alloc.attach_uprobe(
                true, // retprobe
                pid as i32,
                exe_path,
                alloc_offset as usize,
        ...
        if let Some(dealloc_offset) = symbols.rust_dealloc {
            let link = skel.progs.uprobe_rust_dealloc.attach_uprobe(
                false,
                pid as i32,
                exe_path,
                dealloc_offset as usize,
        ...
        if let Some(realloc_offset) = symbols.rust_realloc {
            // Entry
            let link = skel.progs.uprobe_rust_realloc_v2.attach_uprobe(
                false,
                pid as i32,
                exe_path,
                realloc_offset as usize,
            ).map_err(|e| Error::Bpf(format!("Failed to attach realloc uprobe: {}", e)))?;
            links.push(link);

            // Return
            let link = skel.progs.uretprobe_rust_realloc.attach_uprobe(
                true,
                pid as i32,
                exe_path,
                realloc_offset as usize,
                ...
                if value.len() >= 265 {
                    ...
                    if stack_len > 0 && stack_len <= 32 {
                        ...
                        for i in 0..stack_len {
                            let offset = 8 + i * 8;
                            let addr = u64::from_ne_bytes(value[offset..offset + 8].try_into().unwrap());
                            if addr != 0 {
                                stack.push(addr);
                            }
                        }
    ...
    pub fn poll_events(&mut self, timeout: Duration) -> Vec<HeapEvent> {
        ...
        if builder.add(&self.skel.maps.events, callback).is_ok() {
            if let Ok(ring_buffer) = builder.build() {
...
fn find_allocator_symbols(exe_path: &Path) -> Result<AllocatorSymbols> {
    ...
    let data = std::fs::read(exe_path).map_err(|e| {
        Error::Bpf(format!("Failed to read executable: {}", e))
    })?;

    let file = object::File::parse(&*data).map_err(|e| {
        Error::Bpf(format!("Failed to parse executable: {}", e))
    })?;
    ...
    for sym in file.symbols() {
        if let Ok(name) = sym.name() {
            ...
            if name == "__rust_alloc" || name.contains("___rust_alloc") {
                ...
                if symbols.rust_realloc.is_none() {
    ...
    for sym in file.dynamic_symbols() {
        if let Ok(name) = sym.name() {
            let vaddr = sym.address();
            if (name == "__rust_alloc" || name.contains("___rust_alloc")) && symbols.rust_alloc.is_none() {
                symbols.rust_alloc = vaddr_to_file_offset(&file, vaddr);
            } else if (name == "__rust_dealloc" || name.contains("___rust_dealloc")) && symbols.rust_dealloc.is_none() {
                symbols.rust_dealloc = vaddr_to_file_offset(&file, vaddr);
            } else if (name == "__rust_realloc" || name.contains("___rust_realloc")) && symbols.rust_realloc.is_none() {
                symbols.rust_realloc = vaddr_to_file_offset(&file, vaddr);
            }
        }
    }
    ...
    if symbols.rust_alloc.is_none() && symbols.rust_dealloc.is_none() {
        return Err(Error::Bpf(
            "No Rust allocator symbols found. Target may not be a Rust binary or may be stripped.".to_string()
        ));
    }
